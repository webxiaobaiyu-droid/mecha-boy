...
import { ENDING, INTRO, TRACKS } from '@/game/data/story'
...
export function renderTitle(
  ...
  root.addChild(coverSprite(titleTexture, width, height))
  const shade = new Graphics()
    .rect(0, 0, width, height)
    .fill({ color: 0x0b0909, alpha: 0.28 })
    .rect(0, 0, Math.min(width, 720 * scale), height)
    .fill({ color: 0x090707, alpha: 0.7 })
    .rect(0, height * 0.7, width, height * 0.3)
    .fill({ color: 0x090707, alpha: 0.76 })
  root.addChild(shade)

  const edge = uiEdgeFor(width, height)
  const compact = width < 720
  const logoSize = Math.max(42, Math.min(76, 62 * scale))
  const logo = label(
    '荒原引擎',
    edge * 1.5,
    Math.max(edge, height * 0.09),
    displayStyle(logoSize, {
      fill: UI_COLORS.paper,
      stroke: { color: 0x321b16, width: Math.max(3, 5 * scale) },
      dropShadow: {
        color: 0x080707,
        alpha: 0.9,
        blur: 0,
        angle: Math.PI / 4,
        distance: Math.max(3, 5 * scale)
      }
    })
  )
  fitText(logo, width - edge * 3)
  root.addChild(logo)
  const subtitle = label(
    '每一台旧机器，都有下一段路。',
    edge * 1.7,
    Math.max(edge, height * 0.09) + logoSize + 12 * scale,
    bodyStyle(Math.max(13, 15 * scale), { fill: UI_COLORS.paper })
  )
  root.addChild(subtitle)

  if (state.creditsOpen) {
    const cardWidth = Math.min(width - edge * 2, 720 * scale)
    const cardHeight = Math.min(height - edge * 2, 470 * scale)
  ...
  const menuItems = ['开始新游戏', '继续冒险', '音乐室', '制作名单']
  const menuWidth = Math.min(compact ? width - edge * 3 : 310 * scale, width - edge * 2)
  const rowHeight = Math.max(42, 48 * scale)
  const menu = panel(menuWidth, rowHeight * menuItems.length + 26 * scale, {
    alpha: 0.82,
    accent: UI_COLORS.warning,
    label: 'IGNITION'
  })
  menu.position.set(edge * 1.5, height - edge * 1.5 - menu.height)
  menuItems.forEach((item, index) => {
    const row = uiButton(item, menuWidth - 20 * scale, rowHeight, {
      active: state.titleMenu === index,
      disabled: index === 1 && !store.hasSave(),
      value: index === 0 ? 'NEW' : index === 1 ? 'LOAD' : index === 2 ? 'PLAY' : 'INFO',
      onPress: () => store.titleChoose(index)
    })
    row.position.set(10 * scale, 12 * scale + index * rowHeight)
    menu.addChild(row)
  })
  root.addChild(menu)
  return root
